...
class Compose(object):
    ...
    def __call__(self, img, pts=None, craft=None):
        for t in self.transforms:
            img, pts, craft = t(img, pts, craft)
        return img, pts, craft

class Compose_eval(object):
    """Composes several augmentations together.
    Args:
        transforms (List[Transform]): list of transforms to compose.
    Example:
...
class Normalize(object):
    ...
    def __call__(self, image, polygons=None, crafts=None):
        image = image.astype(np.float32)
        image /= 255.0
        image -= self.mean
        image /= self.std
        return image, polygons, crafts

class Normalize_eval(object):
    def __init__(self, mean, std):
        self.mean = np.array(mean)
        self.std = np.array(std)
...
class RandomMirror(object):
    ...
    def __call__(self, image, polygons=None, crafts= None):
        ...
        if np.random.randint(2):
            ...
            for craft in crafts:
                craft.points[:,0] = width - craft.points[:, 0]
        return image, polygons, crafts
...
class RotatePadding(object):
    ...
    def __call__(self, image, polygons=None, crafts=None, scale=1.0):
        ...
        if crafts is not None:
            for craft in crafts:
                x, y = self.rotate(center, craft.points, angle, movSize, scale)
                pts = np.vstack([x, y]).T
                craft.points = pts                
        return image, polygons, crafts
...
class RandomCropFlip(object):
    ...
    def __call__(self, image, polygons=None, crafts=None):
        ...
        _, _, pp_polys_craft = crop_first(image, crafts, scale =self.scale)
        ...
        while attempt < 10:
            ...
            for craft in crafts:
                ppi = Polygon(craft.points).buffer(0)
                ppiou = float(ppi.intersection(pp).area)
                if np.abs(ppiou - float(ppi.area)) >self.epsilon  and np.abs(ppiou)> self.epsilon:
                    Fail_flag = True
                    break
                elif np.abs(ppiou - float(ppi.area)) <self.epsilon:
                    polygons_new_craft.append(craft)
                else:
                    pass
        ...
        if len(polygons_new) == 0:
            ...
            if select_type == 0:
                ...
                for craft in polygons_new_craft:
                    craft.points[:, 0] = width - craft.points[:, 0] + 2 * xmin
                ...
                for craft in polygons_new_craft:
                    craft.points[:, 1] = height - craft.points[:, 1] + 2 * ymin
            else:
                img = np.ascontiguousarray(cropped[::-1, ::-1])
                for polygon in polygons_new:
                    polygon.points[:, 0] = width - polygon.points[:, 0] + 2 * xmin
...
class RandomResizedCrop(object):
    ...
    def __call__(self, image, polygons, crafts):
        ...
        while attempt < 10:
            ...
            xmax = np.clip(xmax, 0, w - 1)
            yy = np.random.choice(h_axis, size=2)
            # print("random",np.random.randint(1,100))
            ymin = np.min(yy) - pad_h
            ymax = np.max(yy) - pad_h
            ymin = np.clip(ymin, 0, h - 1)
            ymax = np.clip(ymax, 0, h - 1)
            ...
            if len(selected_polys) == 0:
                ...
                for polygon in copy.deepcopy(polygons):
                    ...
                    if np.abs(ppiou - float(ppi.area)) > self.epsilon and np.abs(ppiou) > self.epsilon:
                        ...
                        # polygon.points -= np.array([xmin, ymin])
                        polygons_new.append(polygon)
                for craft in copy.deepcopy(crafts):
                    ppi = Polygon(craft.points).buffer(0)
                    ppiou = float(ppi.intersection(pp).area)
                    if np.abs(ppiou - float(ppi.area)) > self.epsilon and np.abs(ppiou) < self.epsilon:
                        Fail_flag = True
                ...
                if Fail_flag:
                    ...
                    for polygon in polygons_new:
                        polygon.points -= np.array([xmin, ymin])
                    for craft in polygons_new_craft:
                        craft.points -= np.array([xmin, ymin])

                    return cropped, polygons_new, polygons_new_craft

        return image, polygons, polygons_new_craft
...
class RandomResizeScale(object):
    ...
    def __call__(self, image, polygons=None, crafts = None):
        ...
        if crafts is not None:
            for craft in crafts:
                craft.points = craft.points * scales
...
class ResizeLimitSquare(object):
    ...
    def __call__(self, image, polygons=None, crafts=None):
        ...
        if np.random.random() <= self.ratio:
            image, polygons, crafts = self.SP(image, polygons, crafts)
        h, w, _ = image.shape
        image = cv2.resize(image, (self.size,self.size))
        scales = np.array([self.size*1.0/ w, self.size*1.0 / h])
        
        def scale_points(objs):
            if objs is not None:
                for obj in objs:
                    obj.points = obj.points * scales
            return objs
        polygons = scale_points(polygons)
        crafts = scale_points(crafts)
        """
        if polygons is not None:
            for polygon in polygons:
                polygon.points = polygon.points * scales
...
class Augmentation(object):
    ...
    def __call__(self, image, polygons=None, crafts = None):
        ...
        return self.augmentation(image, polygons, crafts)
...
class BaseTransform(object):
    def __init__(self, size, mean, std):
        ...
        self.mean = mean
        self.std = std
        self.augmentation = Compose_eval([
            # Resize(size),
            ResizeSquare(size=self.size),
            Normalize_eval(mean, std)
        ])

...
def get_craft_results(i):
    with open("craft_ctw_test.pkl","rb") as f:
        craft_results = pickle.load(f)
        craft_result = craft_results[i]
...
def inference(model, test_loader, output_dir):
    ...
    for i, (image, meta) in enumerate(test_loader):
        ...
        input_dict["craft"] = get_craft_results(int(meta['image_id'][0][:4])-1001)
        #input_dict["img_shape"] = get_img_shape(i)
        input_dict["meta_HW"]=np.array([int(meta['Height']),int(meta['Width'])])

        # print("img", input_dict['img'].shape)
        # print("craft", input_dict['craft'])
        # print("input_dict", input_dict['meta_HW'])
        ...
        print("idx", idx)
        print("meta['image_id']",meta['image_id'])
        # visualization
...
def main(vis_dir_path):
    ...
    test_loader = data.DataLoader(testset, batch_size=1, shuffle=True, num_workers=cfg.num_workers)

    # Model
    print("cfg.net",cfg.net)

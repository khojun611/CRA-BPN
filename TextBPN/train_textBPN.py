...
from using_craft import train_get14p

from dataset import SynthText, TotalText, Ctw1500Text, Icdar15Text, Mlt2017Text, TD500Text
from network.loss import TextLoss
from network.textnet import TextNet
from util.augmentation import Augmentation, BaseTransform, BaseTransformNresize
...
# train_get14p("./ctw_train_coord")
#with open("craft_ctw_train.pkl","rb") as f:
#    craft_results = pickle.load(f)

def select_craft_results(image_id):
    ...
    for i in image_id_list:
        b.append(int(i[:4]))
    craft_result = []
    binary_array = torch.zeros(12,200)
    for idx,j in enumerate(b):
        craft_result.extend(craft_results[j-1])
        # print("input_number",len(craft_results[j-1]))
        input_number = len(craft_results[j-1])
        binary_array[idx, :input_number] = 1
    # print("len craft result",np.array(craft_result).shape)
    # print("craft_result",craft_result)
    
    
    return craft_result, binary_array

"""
def craft_ignore_tags(image_id):
    b = []
    for i in image_id:
        b.append(int(i[:4]))
    craft_result = []
...
def _parse_data(inputs):
    input_dict = {}
    input_others=inputs[9:]
    inputs = list(map(lambda x: to_device(x), inputs[:9]))
    ...
    input_dict['image_id'] = input_others[0]
    # input_dict['craft'],_ = select_craft_results(input_others[0])
    # _, input_dict['craft_ignore_tags'] = select_craft_results(input_others[0]).float()
    # input_dict["H"] = input_others[1]
    # input_dict["W"] = input_others[2]
    input_dict["craft_coords"] = input_others[3].float()
    # input_dict['image_id'] = input_others[3]
    return input_dict
...
def train(model, train_loader, criterion, scheduler, optimizer, epoch):
    ...
    for i, inputs in enumerate(train_loader):
        ...
        #input_dict['craft'] = get_craft_results(inputs[9])
        #input_dict["meta_HW"]=np.array([int(inputs[10]),int(inputs[11])])
        #input_dict['craft'] = select_craft_results(input_dict['image_id'])
        # print("input_dict['proposal points']",len(input_dict["proposal_points"][0][0]))
        # print(input_dict["proposal_points"][0])
        # print("input_dict['gt_points]",len(input_dict['gt_points'][0][0]))
        # print(input_dict['gt_points'][0])
    ...
    if epoch % cfg.save_freq == 0 and epoch > 20:

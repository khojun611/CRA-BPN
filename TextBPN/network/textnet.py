...
class Evolution(nn.Module):
    ...
    def get_boundary_proposal(input=None, seg_preds=None, switch="gt"):
        ...
        if switch == "gt":
            # inds = torch.where(input['craft_ignore_tags'] > 0)
            # print("inds",inds)
            inds = torch.where(input['ignore_tags'] > 0)
            
            # print("inds",inds)
            # print("inds",inds)
            # print("input ignore",input['ignore_tags'])
            # print("input ignore shape",input['ignore_tags'].shape)
            # print("inds shape",inds.shape)
            # print("image id list",input['image_id'])
            ...
            """
            # print("inds")
            print("image id",input['image_id'])
            array = np.array(init_polys.cpu())
            print("array shape",array.shape)
            np.save('/home/iris/hojun/craft_plus_textbpn/TextBPN/my_array.npy',array)
            print("save done")
            print("init polys shape",init_polys.shape)
            # print("init polys",init_polys)
            # print("proposal_points shape",input['proposal_points'].shape)
            # print("init poly shape",init_polys.shape)
            """
            
            
            
            
            # init_polys = np.array(input["craft"])
            
            init_polys = input["craft_coords"][inds]
            # init_polys = torch.from_numpy(np.array(init_polys)).to(input["img"].device)
            
            # print("init_polys shape",init_polys.shape)
            # print(init_polys)
            # print("craft",np.array(input["craft"]).shape)
            init_polys = torch.from_numpy(np.array(init_polys)).to(input["img"].device)
            # np.save('array_file.npy', init_polys)
            # print("init polys shape",init_polys.shape)
            # inds = torch.zeros(2,len(init_polys),dtype=torch.int64)
            # print("inds shape",inds.shape)
            # print("inds",inds)
    ...
    def get_boundary_proposal_eval(input=None, seg_preds=None):
        ...
        # print("dis_preds shape",dis_preds.shape)
        ...
        for bid, dis_pred in enumerate(dis_preds):
            ...
            # print("dismaks shape:",dis_mask.shape)
        ...
        #print("init polys:",init_polys)
        # print("label shape:",labels.shape)
        # print("meta_HW ",np.array(input["meta_HW"]))
        
        img_scale_mk =labels.shape/input["meta_HW"]
        # print("labels shape",labels.shape)
        img_scale_mk=img_scale_mk[::-1]
        # print("img_scale_mk",img_scale_mk)
        # print("init_polys1",np.array(input["craft"]))
        init_polys = torch.tensor(np.array(input["craft"]))
        # init_polys = torch.tensor(np.array(input["craft"])*img_scale_mk)
        # print("init polys2",init_polys)
        # print("craft_init_poly:",init_polys)
        #print("imgshape ",input["img_shape"])
...
class TextNet(nn.Module):
    ...
    def __init__(self, backbone='vgg', is_training=True):
        ...
        self.BPN = Evolution(cfg.num_points, adj_num=4,
                             is_training=is_training, device=cfg.device, model="rnn")
    ...
    def forward(self, input_dict):
        ...
        print("img shape",input_dict["img"].shape)
        ...
        # print("cnn feat shape",cnn_feats.shape)
        ...
        # print("py pred shape",len(py_preds))
        # print("py_preds[0] shape",len(py_preds[0]))
        # print("fy pred shape",fy_preds.shape)
        # print("initpolys shape",init_polys.shape)
        output["inds"] = inds
